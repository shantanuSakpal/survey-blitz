...
export default function ResponsesTable({
  ...
  useEffect(() => {
    const transformedResponses = responsesArray.map((response, index) => {
      ...
      response.formObject.form_sections.forEach((section, index) => {
        ...
        section.section_components.forEach((component, index) => {
          const question =
            formSection?.section_components[index]?.component_prop_object
              ?.question;
          transformedResponse[question] =
    ...
    const uniqueColumns = Array.from(
      new Set(transformedResponses.flatMap((response) => Object.keys(response)))
    ).map((key) => ({
      id: id++,
      header: key,
      accessorKey: key,
